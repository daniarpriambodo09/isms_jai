...
type TitleGroup = { key: string; title: string; docs: StandardIsmsP14Document[] }
...
function groupByTitle(docs: StandardIsmsP14Document[]): TitleGroup[] {
...
export function StandardIsmsP14RegisterPage() {
  ...
  const [pendingDelete, setPendingDelete] = useState<StandardIsmsP14Document | null>(null)
  ...
  const loadDocuments = useCallback(async () => {
    setLoading(true)
    try {
      const response = await fetch(`${API_BASE_PATH}/api/standard-isms-p14`, { cache: 'no-store' })
  ...
  const confirmBulkDelete = async () => {
    ...
    const results = await Promise.all(ids.map((id) => fetch(`${API_BASE_PATH}/api/standard-isms-p14?id=${id}`, { method: 'DELETE' })))
  ...
  const confirmDelete = async () => {
    ...
    setDeleting(true)
    try {
      const response = await fetch(`${API_BASE_PATH}/api/standard-isms-p14?id=${pendingDelete.id}`, { method: 'DELETE' })
      if (!response.ok) {
  ...
  const editableDocument: EditableStandardIsmsP14 | undefined = editing ? {
    id: editing.id,
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <StandardIsmsP14FormModal open={formOpen} onClose={() => setFormOpen(false)} onSaved={loadDocuments} document={editableDocument} />
      <ConfirmDialog
        open={!!pendingDelete}
        title="Hapus dokumen?"
