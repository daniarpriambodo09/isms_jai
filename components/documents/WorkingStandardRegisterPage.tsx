...
type TitleGroup = { key: string; title: string; docs: WorkingStandardDocument[] }
...
// control no./revision/date/action listed together.
function groupByTitle(docs: WorkingStandardDocument[]): TitleGroup[] {
...
export function WorkingStandardRegisterPage() {
  ...
  const confirmBulkDelete = async () => {
    ...
    const results = await Promise.all(ids.map((id) => fetch(`${API_BASE_PATH}/api/working-standard?id=${id}`, { method: 'DELETE' })))
        ...
        {groupByTitle(pageItems).map((group, index) => (
          <tr key={group.key} className={`table-row-glow ${index % 2 ? 'bg-secondary/20' : ''}`}>
            ...
            <td className="px-5 py-4 align-top text-muted-foreground">
              <div className="flex flex-col gap-1.5">
                {group.docs.map((document) => <div key={document.id} className="whitespace-nowrap py-0.5">{formatEffectiveDate(document.effective_date)}</div>)}
              </div>
            </td>
              ...
              <div className="flex flex-col gap-1.5">
                {group.docs.map((document) => (
                  <div key={document.id} className="flex items-center gap-1 py-0.5">
                    ...
                    {isLoggedIn && <>
                      <button type="button" onClick={() => { setEditing(document); setFormOpen(true) }} aria-label={`Edit ${document.title}`} title="Edit dokumen" className="grid size-8 place-items-center rounded-md text-muted-foreground hover:bg-secondary hover:text-accent-foreground"><Pencil className="size-4" /></button>
